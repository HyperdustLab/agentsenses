...
metadata:
  ...
  rationale: 'instructions in HEARTBEAT.md can be deprioritised by the model; this concern injects the inquiry directly into the prompt every turn'
---

# Self-inquiry carrier

When the chat-self-heartbeat pipeline has an open inquiry, this concern injects a compact `<self_inquiry_reminder>` block into the system context on every turn. The agent cannot "forget" to ask while an inquiry is open — the reminder is part of its prompt.

If there are no open inquiries, the concern returns nothing and adds zero tokens.
