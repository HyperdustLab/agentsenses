/**
 * Executable advice for the self_inquiry_carrier sense.
 *
 ...
 * zero tokens.
 *
 * Return fields (picked up by openclaw-senses-plugin): prependSystemContext,
 ...
 *       The sense NEVER writes to disk and NEVER mutates inquiries.json.
 */
...
function renderBanner(primary, allOpen) {
  ...
  const requiredAction = primary.escalate_to_web
    ...
    : [
        ...
        `When the user replies, close the inquiry and create a tier-${tier} Sense:`,
        `    node ~/.openclaw/workspace/skills/chat-self-heartbeat/scripts/tick.js --answer ${primary.id}`,
