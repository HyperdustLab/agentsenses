---
name: concern-test-web-answer
description: Regression playbook — web search path and how web-answer, citation, and safety stack on answer shape.
---

# Concern regression: web + citations + safety (execution)

This playbook walks **one user thread** through several concerns so you can compare “model without crosscuts” vs “with crosscuts.”

## Prerequisites

- Keep `examples/concerns/web-answer`, `examples/concerns/citation_sense`, `examples/concerns/safety_sense` (and plugin NOTICE behavior).  
- Optional: temporarily move aside `examples/concerns/example_*` to reduce noise.

## Mental model

- **Crosscuts off:** long answers, weak sourcing, under-cautious on sensitive topics.  
- **Crosscuts on:** **NOTICE** if rules require; `web-answer-concern` asks for **concise + sources first**; `citation_sense` asks for **evidence on facts**; `safety_sense` tightens wording when it matches.
...
| Check | If concerns fire, you should see |
| ----- | ------------------------------ |
| NOTICE | Early paragraph may show `NOTICE: The following concern rules were triggered…` (exact text per plugin). |
| Length | Relatively short; not an essay (`web-answer-concern`). |
...
**Step 1**  
In the Concern Client UI select skill **`web_answer`** (or equivalent routing), then send:  
...
In Gateway logs search `weave @execution` and concern names to see **which concerns matched the same turn**.
