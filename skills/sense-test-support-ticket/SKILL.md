...
- Keep `examples/senses/example_initialization` (`any_of`: **support ticket**, **customer**).  
...
**Observe**  
Whether `initialization` still matches depends on whether the Sense Client runs **`before_agent_start`** on the second turn (often only on first start). If it does not match, open a **new session** and rerun sub-experiment A for a canonical demo.
