/**
 * Persist a confirmed concern candidate to the workspace `concerns/` directory.
 *
 * Spec: specification/CHAT_DEVELOPED_AGENT.md §4.4 (Persister) and §8 (IEM).
 *
 * Writes:
 *   <sensesRoot>/<tier>/<name>/CONCERN.md
 *
 * Invariants:
 ...
 *   - refuses to overwrite an existing stable/core concern without a `replace` flag
 */
...
function assertInsideRoot(abs, root) {
  ...
  if (a !== r && !a.startsWith(r + path.sep)) {
    throw new Error(`refused: path ${a} escapes concerns root ${r}`);
  }
}
...
function assertSafeName(name) {
  if (typeof name !== "string" || !/^[a-z0-9]([a-z0-9_-]{0,62}[a-z0-9])?$/.test(name)) {
    throw new Error(`refused: invalid concern name "${name}"`);
  }
}
...
function detectExistingState(sensesRoot, tier, name) {
  ...
  const file = path.join(dir, "CONCERN.md");
...
function persist(args) {
  ...
  const targetFile = path.join(targetDir, "CONCERN.md");
  ...
  if (existing && !replace) {
    if (existing.state === "stable" || existing.state === "core") {
      throw new Error(
        `refused: concern "${name}" is ${existing.state}; use Confirmer merge/replace flow`
      );
    }
  }
...
/**
 * Move a concern directory to `.archive/` (soft delete, preserves history).
 */
function archive({ sensesRoot, tier, name, approver = "user" }) {
  ...
  if (!fs.existsSync(src)) {
    throw new Error(`concern "${tier}/${name}" does not exist`);
  }
