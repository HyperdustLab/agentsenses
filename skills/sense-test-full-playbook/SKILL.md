---
name: sense-test-full-playbook
description: Test coach — staged walkthrough for a human to validate sense crosscuts (what to say, what to watch, how to mark pass/skip/fail).
---

# Sense test master playbook (coach-facing)
...
4. If the user only says “run the sense crosscut master playbook” or “run the master skill,” your **first reply** may only:  
...
When `/skill sense-test-full-playbook` or equivalent is invoked, prefer something like:

> We’ll run a small **OpenClaw sense crosscut** experiment in stages.  
...
1. Plugin `@local/openclaw-senses` is on; has the Gateway been restarted recently?  
2. Are you testing the workspace `senses/` (e.g. `~/.openclaw/workspace`), with example packs copied from this repo’s `examples/senses/` (or your own equivalents)?  
3. If they use logs: can they `tail` and grep `weave @`?  
4. Note: if **NOTICE** is noisy, temporarily move aside `examples/senses/example_*` you’re not testing (or the copies you placed under workspace `senses/`).
...
**Sub-skill:** `sense-test-prod-coldstart`
...
**Sub-skill:** `sense-test-compliance-coldstart`
...
**Sub-skill:** `sense-test-support-ticket`
...
**Sub-skills:** `sense-test-web-answer`, `sense-test-crypto-wallet`
...
**Sub-skill:** `sense-test-external-publish`
...
**First:** if the Sense Client **never** uses `llm_input`, there may be no `weave @get` — **not** a user failure.  
...
**Sub-skill:** `sense-test-bank-balance`
...
**Sub-skill:** `sense-test-price-reply`
...
**Risk:** only in an **isolated workspace** with **no important data** and permission to overwrite test files; or use the Sense Client’s “fake tool / sandbox” paths.  
...
**Sub-skill:** `sense-test-destructive-tool`
...
**Sub-skill:** `sense-test-error-path`
...
**Sub-skill:** `sense-test-reserved-joints`
...
|-------|------------------|
| Web / citation / safety | `sense-test-web-answer`, `sense-test-crypto-wallet` |
| Prod cold start | `sense-test-prod-coldstart` |
| Compliance cold start | `sense-test-compliance-coldstart` |
| Support ticket | `sense-test-support-ticket` |
| External publish | `sense-test-external-publish` |
| Balance / get | `sense-test-bank-balance` |
| Quote / set | `sense-test-price-reply` |
| Tool / call | `sense-test-destructive-tool` |
| Error / handler | `sense-test-error-path` |
| Reserved | `sense-test-reserved-joints` |
