...
- Keep `examples/senses/example_execution` (`any_of` includes **external publish**).  
- The user message must contain the phrase **`external publish`** (or a synonym that still substring-matches the haystack); otherwise this sense does not fire.
...
**Observe**  
`example_execution` **should not** reshape structure just from this sense’s pointcut; if the answer is still careful, that is model baseline, not this pointcut.
