...
- Keep `examples/concerns/example_call` (`jointpoint == "call"`, `any_of`: **delete**, **overwrite**).  
- Needs a **real tool call** whose `toolName` / `params` strings can match those substrings.

## Sub-experiment A: a call the concern should “name-check”

With operator / Concern Client permission, have the assistant run something like:  
“**Delete** `tmp/demo.txt` under the workspace (if missing, just say so).”

**Observe (assistant side, if the concern is in readable context)**  
...
If there **is** a match, the **call** jointpoint pointcut fired; the chat may not show a big `<Concerns>` block — that is a **Concern Client hook limitation**, not a missing concern.
