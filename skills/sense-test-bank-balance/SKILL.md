...
- Keep `examples/senses/example_get` (`jointpoint == "get"` and `any_of` includes **balance**).  
- **Critical:** the Sense Client must actually emit **`llm_input`**; if it never does, this sense **never** matches (chat content won’t matter).
...
- Stress **permission / audit / refuse if unauthorized** (aligned with sense body).  
...
## Sub-experiment 3: mix with execution senses in one session
