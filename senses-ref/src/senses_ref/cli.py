...
def main():
    """Reference tools for Agent Senses (SENSE.md packages)."""
    pass
...
def validate_cmd(path: Path):
    """Validate a single sense package directory (or parent of SENSE.md if a file is given)."""
    if path.is_file():
        if path.name.lower() == "sense.md":
            path = path.parent
        else:
            click.echo(f"Not a SENSE.md file: {path}", err=True)
            sys.exit(1)
    ...
    if errors:
        ...
        sys.exit(1)
    click.echo(f"Valid sense package: {path}")


@main.command("validate-tree")
@click.argument("root", type=click.Path(exists=True, path_type=Path))
def validate_tree_cmd(root: Path):
    """Validate every sense package in immediate subdirectories of ROOT (e.g. examples/senses)."""
    bad = validate_tree(root)
    if not bad:
        click.echo(f"All sense packages under {root} are valid.")
        return
...
def read_properties_cmd(path: Path):
    ...
    try:
        if path.is_file() and path.name.lower() == "sense.md":
            path = path.parent
