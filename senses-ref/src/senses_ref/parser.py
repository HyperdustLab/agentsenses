"""Parse SENSE.md and legacy sense.yaml + prompt.md."""
...
def parse_frontmatter(content: str) -> tuple[dict[str, Any], str]:
    """Split YAML frontmatter and Markdown body (matches openclaw-senses-plugin behavior)."""
    text = content.replace("\ufeff", "", 1)
    if not text.startswith("---"):
        raise ParseError("SENSE.md must start with YAML frontmatter (---)")

    after_open = text[3:].lstrip("\r\n")
    m = _CLOSING_FRONTMATTER.search(after_open)
    if not m:
        raise ParseError(
            "SENSE.md frontmatter must be closed with a line containing only ---"
        )
    ...
    if not isinstance(metadata, dict):
        raise ParseError("SENSE.md frontmatter must be a YAML mapping")
...
def _load_legacy(sense_dir: Path) -> tuple[dict[str, Any], str]:
    meta_path = sense_dir / "sense.yaml"
    prompt_path = sense_dir / "prompt.md"
    if not meta_path.is_file() or not prompt_path.is_file():
        raise ParseError("Legacy layout requires sense.yaml and prompt.md")
    try:
        meta = yaml.safe_load(meta_path.read_text())
    except yaml.YAMLError as e:
        raise ParseError(f"Invalid YAML in sense.yaml: {e}") from e
    if not isinstance(meta, dict):
        raise ParseError("sense.yaml must be a YAML mapping")
